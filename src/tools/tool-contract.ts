...
export const LANE_TOOLS: readonly LogicalToolDefinition[] = [
  { name: "lane_directory", description: "List the lanes in one project with their role descriptions, current binding, and reachability. Call it when a lane does not respond: `reach` separates a lane with no live channel from one whose channel is open but has never reported a turn, and its timestamps show whether notifications are actually waking the target. This query does not require user confirmation. A lane that has been retired is not listed here: retiring, returning to service, and listing the retired ones are shell commands - `lane-router-lane retire|unretire|list-retired`.", mutating: false },
