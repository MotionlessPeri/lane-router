...
这条基线**不包含**：读取或推断某次对话实际在用的模型、模型名合法性校验、按项目或全局设默认、Codex 侧的模型选择、运行中改模型。
...
### 3.5 只作用于 claude backend

`--model` 是 claude 的参数。Codex 有自己的模型选择，本设计不碰：`childCommand` 的 codex 分支原样不动。lane 上声明了模型而 backend 是 codex 时，该值被忽略（不报错——它可能是将来切 backend 用的）。
...
| Codex 侧的模型选择 | —— | 无。不同机制，不同参数 | 无 | —— | Delete |
...
4. `model` 非空时，claude 的两种模式都插入 `--model <值>`；codex 的两种模式**都不插入**。
...
8. 不新增 RPC 方法、HTTP 端点、数据表；工具数量与名称不变。
...
- **参数构造**：`childCommand` 是纯函数，直接断言四种组合（claude/codex × prompt/resume）在 model 为 null 与非空时的完整参数数组。**这是本设计的牙**——第 3 条（null 时逐字不变）尤其重要，缺了它，实施成「总是插入 `--model undefined`」也能让其余测试通过。
...
**变异检验**（三态：变异前绿、变异后红、还原后绿，且每个变异只杀掉对应的那一条）：把「null 时不插入」改成「总是插入」、把 codex 分支也插入、把 attach 的「省略即不改」改成「省略即清空」。
