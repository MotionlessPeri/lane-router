...
⇒ 解不是绕开外键，是**承认有些字段不是引用**。但要拆的只有**两条**，不是全部——判据是**这个字段会不会指向「已经走了、而引用还留在活表里」的东西**：

| 字段 | 外键 | 理由 |
|---|---|---|
| `sender_lane` | ❌ **拆** | 归档 A 之后，A 发给活着的 B 的信留在活表里，sender 指向已消失的 A。**2637 封** |
| `reply_to` | ❌ **拆** | 活着的 B 的回信指向被挪走的信。**831 封** |
| `target_lane` | ✅ 留 | 归档**带走**全部 `target=A` 的行，所以活表里的 target 永远指向活着的 lane |
| `ack_lane` | ✅ 留 | 结构上恒等于 `target_lane`（见下），跟着一起走 |
| `binding.lane_address` | ✅ 留 | binding 确实属于一条活着的 lane，归档时跟着走 |

只有 `sender_lane` 与 `reply_to` 指向的是**别人手里那份历史**，其余三条指向的东西都会跟着一起搬走。
...
7. **`ack_lane` 是冗余列。** ack 只能由收件方做——`markMessagesResolved` 的 `UPDATE ... SET ack_lane=? WHERE ... target_lane=?` 两个占位符绑的是同一个值，所以它**结构上不可能**跟 `target_lane` 不同；实测 4642 条已 ack 的行**零条**不同。它承载的信息 `state='resolved'` 加 `target_lane` 已经全有了（`ack_generation` 不冗余，那是新信息）。🟢 **本轮不动**——拆表已经够大；记在这里，将来简化时它是第一个候选。
