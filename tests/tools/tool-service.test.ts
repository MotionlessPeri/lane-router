...
test("exports exactly the four V1 tools", () => {
  expect(LANE_TOOL_NAMES).toEqual(["lane_directory", "lane_attach_current", "lane_send", "lane_ack"]);
  expect(LANE_TOOLS.find((tool) => tool.name === "lane_attach_current")?.description).toMatch(/explicit confirmation/i);
});

test("validates and dispatches all four tools with authoritative caller context", async () => {
  const x = setup();
